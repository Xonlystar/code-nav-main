...
class TagList extends Component<TagListProps> {
  render() {
    ...
    if (
      resource.reviewStatus === reviewStatusEnum.PASS &&
      moment(new Date()).diff(moment(resource.publishTime), 'days') <= 3
    ) {
      extraTags.push(
      ...
      <div className={styles.tagList} style={style}>
        {showReview && (
          <Tooltip title={resource.reviewMessage} defaultVisible>
            <Tag color={reviewStatusInfoMap[resource.reviewStatus].color}>
              {reviewStatusInfoMap[resource.reviewStatus].text}
            </Tag>
          </Tooltip>
        )}
        {tagListView}
