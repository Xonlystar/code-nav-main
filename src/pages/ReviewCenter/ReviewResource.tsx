...
const ReviewResource: React.FC<ReviewCenterProps> = props => {
  ...
  const doSubmit = (values: Record<string, any>) => {
    ...
    setSubmitting(true)
    const res: any = updateResource(resourceId, values)
    if (res) {
              ...
              renderItem={item => {
                ...
                return (
                  <List.Item key={item._id}>
                    <List.Item.Meta
                      title={
                        <a style={{ color: reviewStatusInfo.color }} onClick={() => setResourceId(item._id)}>
                          {item.name}
                        </a>
                      }
        ...
        <Col md={18} xs={24}>
          <Card>
            ...
            >
              ...
              >
                <SelectTags allTags={wholeTagsMap.allTags} groupTags={wholeTagsMap.groupTags} maxTagsNumber={8} />
              </FormItem>
              <FormItem label="图标" name="icon" tooltip={{ title: '正方形图标展示效果最佳', placement: 'topLeft' }}>
                <PicUploader />
              </FormItem>
              <FormItem label="详情" name="detail">
                <Input.TextArea placeholder="详细介绍该资源的作用、用法等" autoSize={{ minRows: 3, maxRows: 6 }} />
              </FormItem>
              <FormItem label="推荐人">
                <div>
